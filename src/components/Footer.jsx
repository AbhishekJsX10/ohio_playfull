...
function Footer() {
  return (
    <div className="w-full h-fit bg-zinc-800">
      <div className="container mx-auto md:flex-row flex flex-col py-10 px-10">
        ...
        <div className="md:w-1/2 w-full flex flex-col justify-between gap-5">
          <div className="links flex flex-col font-NueueMontreal mt-10 md:mt-0">
            <h1 className="font-NueueMontreal text-3xl md:text-4xl mb-4 border-b-[1px] border-zinc-700 pb-4 pr-10 font-semibold">
              PRESENTATIONS
            </h1>
            <p className="font-NueueMontreal text-lg md:text-xl mb-4 font-">L :</p>
            {["Facebook", "Twitter", "Instagram", "LinkedIn"].map(
              (item, index) => (
                <a
                  className="flex items-center gap-2 underline text-md underline-offset-4 decoration-slate-500"
                  key={index}
                >
                  {item}{" "}
